...
class Save extends CheckoutAttribute
{
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            if (in_array($attributeCode, self::FORBIDDEN_ATTRIBUTE_CODES)) {
                $this->messageManager->addErrorMessage(
                    __(
                        'Attribute code "%1" is reserved by system. Please use another code.',
                        $attributeCode
                    )
                );
     ...
     * @return array
     */
    private function getUnserializedOptions($serializedOptions)
    {
        $options = [];
        $optionUnserialized = [];
        foreach ($this->serializer->unserialize($serializedOptions) as $option) {
            parse_str($option, $optionUnserialized);
            $options = array_merge_recursive($options, $optionUnserialized);
