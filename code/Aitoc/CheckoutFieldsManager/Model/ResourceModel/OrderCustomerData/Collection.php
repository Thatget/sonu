...
class Collection extends AbstractCollection
{
    ...
    public function getAitocCheckoutfieldsByOrderId($orderId = null, $allAttributes = false)
    {
        ...
        if ($optionIds) {
            $selectOptions = $connection->select()
                ->from(
                    ['main_table' => $this->getTable('eav_attribute_option')],
                    [
    ...
    public function getSelectByOrderId($orderId = null, $allAttributes = false)
    {
        ...
        $select = $connection->select()->from(
            ['aceas' => $this->getTable('aitoc_checkout_eav_attribute_store')],
            $fromData
        );

        if ($allAttributes) {
            $select->joinLeft(['av' => $this->getTable('aitoc_sales_order_value')], $condition, []);
        } else {
            $select->joinInner(['av' => $this->getTable('aitoc_sales_order_value')], $condition, []);
        }

        $select->joinLeft(['ea' => $this->getTable('eav_attribute')], 'aceas.attribute_id = ea.attribute_id', [])
            ->joinLeft(
                ['eaov' => $this->getTable('eav_attribute_option_value')],
                '( eaov.option_id = av.value AND eaov.store_id IN (' . $storeIdsString . '))',
                []
            )->joinLeft(['acea' => $this->getTable('aitoc_checkout_eav_attribute')], '( acea.attribute_id = aceas.attribute_id)', [])
